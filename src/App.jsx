...
:root{
  --bg:#111111;--white:#1C1C1C;--border:#2A2A2A;--border2:#3A3A3A;
  --text:#F0EDE8;--text2:#A09D98;--text3:#696663;
  --purple:#D4A847;--purple2:#B8912E;--purple-l:#E8C96A;
  --purple-bg:rgba(212,168,71,0.12);--purple-bg2:rgba(212,168,71,0.20);
  --purple-grad:linear-gradient(135deg,#D4A847,#E8C96A);
  --green:#22C55E;--green-bg:rgba(34,197,94,0.10);
  --yellow:#F59E0B;--yellow-bg:rgba(245,158,11,0.10);
  --orange:#F97316;--orange-bg:rgba(249,115,22,0.09);
  --red:#EF4444;--red-bg:rgba(239,68,68,0.08);
  --blue:#3B82F6;--blue-bg:rgba(59,130,246,0.09);
...
const STY_COLORS=['#8B5CF6','#EA580C','#0891B2','#CA8A04','#16A34A','#DB2777','#06B6D4','#DC2626']
...
function Btn({children,onClick,disabled,full,variant='primary',small,style:sx,...r}){
  const S={
    primary:{bg:'var(--purple-grad)',c:'#0F0F0F',b:'none',sh:'0 2px 8px rgba(212,168,71,0.3)'},
  ...
  }[variant]||{bg:'var(--purple-grad)',c:'#fff',b:'none',sh:'0 2px 8px rgba(212,168,71,0.3)'}
...
function Modal({children,onClose}){return<div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(5px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:24}}><div onClick={e=>e.stopPropagation()} className="fade" style={{background:'var(--white)',borderRadius:18,padding:30,maxWidth:500,width:'100%',boxShadow:'var(--shadow-lg)',maxHeight:'88vh',overflowY:'auto'}}>{children}</div></div>}
...
function Sidebar({active,onNav}){
  ...
  return<div style={{width:'var(--sidebar-w)',background:'var(--white)',borderRight:'1.5px solid var(--border)',height:'100vh',position:'fixed',left:0,top:0,display:'flex',flexDirection:'column',zIndex:10,boxShadow:'2px 0 12px rgba(212,168,71,0.06)'}}>
    <div style={{padding:'18px 16px',borderBottom:'1.5px solid var(--border)'}}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <div style={{width:36,height:36,borderRadius:10,background:'var(--purple-grad)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0,boxShadow:'0 2px 8px rgba(212,168,71,0.35)'}}>
          {imgOk
            ?<img src="/images/icono-logo.png" alt="Logo" style={{width:'100%',height:'100%',objectFit:'cover'}} onError={()=>setImgOk(false)}/>
            :<span style={{fontSize:16,fontWeight:900,color:'#0F0F0F'}}>E</span>}
        </div>
        <div><div style={{fontSize:14,fontWeight:800,color:'var(--text)'}}>EME BS Admin</div><div style={{fontSize:10,color:'var(--text3)',fontWeight:500}}>Panel PRO</div></div>
      </div>
    </div>
  ...
  return<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)'}}>
    <div className="fade" style={{background:'var(--white)',borderRadius:20,padding:44,maxWidth:400,width:'100%',boxShadow:'var(--shadow-lg)',border:'1.5px solid var(--border)'}}>
      <div style={{textAlign:'center',marginBottom:32}}>
        <div style={{width:60,height:60,borderRadius:16,background:'var(--purple-grad)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',overflow:'hidden',boxShadow:'0 4px 16px rgba(212,168,71,0.35)'}}>
          {imgOk?<img src="/images/icono-logo.png" alt="Logo" style={{width:'100%',height:'100%',objectFit:'cover'}} onError={()=>setImgOk(false)}/>:<span style={{fontSize:26,fontWeight:900,color:'#0F0F0F'}}>E</span>}
        </div>
        <h1 style={{fontSize:22,fontWeight:900,color:'var(--text)'}}>EME BS Admin</h1>
        <p style={{fontSize:14,color:'var(--text3)',marginTop:4}}>Acceso restringido</p>
      </div>
      <Inp label="Email" type="email" value={em} onChange={e=>setEm(e.target.value)} placeholder="admin@emebs.com" onKeyDown={e=>e.key==='Enter'&&sub()}/>
...
function TeamDropdown({stylists,selected,onChange}){
  ...
  return<div ref={ref} style={{position:'relative'}}>
    <button onClick={()=>setOpen(o=>!o)} style={{display:'flex',alignItems:'center',gap:7,padding:'8px 14px',fontSize:13,fontWeight:600,fontFamily:'inherit',background:'var(--white)',border:'1.5px solid var(--border2)',borderRadius:9,cursor:'pointer',color:'var(--text)',boxShadow:'var(--shadow)'}}>
      👥 Equipo
      {selected.length>0&&<span style={{background:'var(--purple-grad)',color:'#0F0F0F',borderRadius:20,fontSize:11,fontWeight:700,padding:'1px 7px'}}>{selected.length}</span>}
...
function CalendarView({data,onCancel,onApptAdded}){
  ...
  const SLOT_H=26,START_H=9,END_H=20
